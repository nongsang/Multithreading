...
// 스레드를 많이 만들어서 작업을 시키면 성능이 올라가길 기대했다.
// 하지만 스레드가 많을 수록 속도는 더 떨어진다.
// 상호배재가 문제가 된다.
// 락을 얻은 스레드 1개만 연산을 하게 되는데, 그러면 싱글스레드로 연산을 하는 것과 차이가 없어진다.
// 스레드가 많아지면 스레드들이 번갈아가면서 연산을 하기 때문에 다른 스레드에게 연산을 넘겨주는 교환작업도 추가가 된다.
// 이 스레드 교환 작업을 문맥교환(Context Switching)이라고 한다.
// 스레드가 많아질 수록 문맥교환은 많아지며, 싱글스레드와 다를바 없는 작업이기 때문에 성능은 떨어질 수 밖에 없다.