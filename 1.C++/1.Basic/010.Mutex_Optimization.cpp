...
{
	volatile int local_num = 0;	// 지역변수를 생성해서 내부에서 각자 작업을 시킨다.
								// volatile은 최적화를 하지 않아서 메모리에서 값을 읽어오고 처리한 다음 저장한다.
								// 최적화를 하면 바로 결과값을 구할 수 있고 더 빠르지만, 스레드의 갯수에 따라 속도가 빨라지는지 확인하기 위해 사용

	for( auto i = 0; i < 500000000 / num_threads; ++i )
	{
		local_num += 2;	// 지역변수에 병렬로 값을 더한다.
						// volatile을 사용했으므로 메모리를 읽고, 쓰고, 저장하는 과정을 그대로 수행한다.
